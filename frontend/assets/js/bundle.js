...
function hideErrorMassages(form) {
    form
...
function showErrorMassage(input, msg) {
    ...
    const errorMassage = formFields.querySelector('.error-message');
...
function checkForEmptyFields(...inputs) {
    inputs.forEach((input) => {
        if (!input.value)
            showErrorMassage(input, 'Campo não pode ficar vazio');
    });
}
function checkEmail(input) {
    if (!(0, isEmail_1.default)(input.value))
        showErrorMassage(input, 'Email inválido');
}
